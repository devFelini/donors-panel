...
import {TimestampToDate, FormatedPrice} from 'functions'
...
class DailyDetailMainItem extends Component {
	...
	getData() {
		let respons = global.queryAddress+'?action=get_diagram_stat&type='+this.props.info+'&start='+TimestampToDate(this.state.date)+'&end='+TimestampToDate(this.state.date)+"&day=true"
	...
	processingData(data) {
		...
		types.map((elem, i) => {  
			let value = Number(data[elem.id+"_s"])
			types[i].value = value
			total += value
		})
