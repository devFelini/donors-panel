...
import {CountPages} from 'functions'
...
class DailyList extends Component {
	...
	getData(direction = false, page = false) {
		...
		let respons = global.queryAddress+'?action=get_daily_stat&page='+loaded_page
	...
	checkCurrentMonth() {
		...
		list_block.find(".daily__item").each(function(i, elem) {
			...
			if(top > 0) {
				let month = Number($(elem).data('month'))
				let year = Number($(elem).data('year'))

				if(month != this.state.curr_month || year != this.state.curr_year) {
					this.setState({ 
						curr_month: month,
						curr_year: year
					})
	...
	render() {
		...
		return (
			<div className="daily">
				<MonthHead curr_month={this.state.curr_month} curr_year={this.state.curr_year} />
							...
							return (
								<div className="daily__list__page" key={page.page} data-page={page.page}>
									{page.data.map((elem, j) => { 
										...
										return (
											<DailyItem key={elem.date} data={elem} date={date} setMode={this.props.setMode} setItemData={this.setItemData}/>
										)		
